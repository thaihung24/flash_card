...
export const FlashCardComponent: React.FC<FlashCardComponentProps> = ({
  ...
  const handleFlip = () => {
    ...
    console.log(`🔄 Card flipped: ${newFlippedState ? 'Back' : 'Front'} (rotateY: ${newFlippedState ? 180 : 0})`);
  };
    ...
    .onEnd((event) => {
      ...
      if (Math.abs(translationX) > Math.abs(translationY)) {
        ...
        if (Math.abs(translationX) > SWIPE_THRESHOLD || Math.abs(velocityX) > 500) {
          swipeDirection = translationX > 0 ? SwipeDirection.RIGHT : SwipeDirection.LEFT;
        }
      } else {
        // Vertical swipe
        if (Math.abs(translationY) > SWIPE_THRESHOLD || Math.abs(velocityY) > 500) {
          swipeDirection = translationY > 0 ? SwipeDirection.DOWN : SwipeDirection.UP;
        }
      }
      
      if (swipeDirection && !disabled) {
        // Animate card off screen
        const targetX = swipeDirection === SwipeDirection.RIGHT ? screenWidth : 
                        swipeDirection === SwipeDirection.LEFT ? -screenWidth : 0;
        const targetY = swipeDirection === SwipeDirection.DOWN ? screenHeight :
                        swipeDirection === SwipeDirection.UP ? -screenHeight : 0;
    ...
    <View style={styles.container}>
        ...
        <Animated.View style={[styles.card, cardAnimatedStyle]}>
          ...
          <Animated.View style={[styles.cardSide, styles.cardFront, frontAnimatedStyle]}>
            <View style={styles.cardHeader}>
              ...
              <TouchableOpacity
                ...
                onPress={() => handleSpeak(card.front)}
              >
            ...
            >
              <View style={styles.cardContent}>
                <Text style={styles.cardTextFront}>{card.front || 'No front text'}</Text>
                {card.reading && (
            ...
            <TouchableOpacity style={styles.cardFooter} onPress={handleFlip} activeOpacity={0.7}>
          ...
          <Animated.View style={[styles.cardSide, styles.cardBack, backAnimatedStyle]}>
            ...
            >
              ...
              <View style={styles.cardContent}>
                <Text style={styles.cardTextBack}>{card.back || 'No back text'}</Text>
                <Text style={styles.originalText}>{card.front || 'No front text'}</Text>
              </View>
      ...
      <View style={styles.swipeIndicators}>
        <View style={[styles.indicator, styles.leftIndicator]}>
          ...
          <Text style={[styles.indicatorText, { color: COLORS.error }]}>Hard</Text>
        </View>
        <View style={[styles.indicator, styles.rightIndicator]}>
          <Ionicons name="checkmark-circle" size={24} color={COLORS.success} />
          <Text style={[styles.indicatorText, { color: COLORS.success }]}>Easy</Text>
        </View>
      </View>
    </View>
  );
