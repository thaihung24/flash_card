...
export const QuickCategoryScreen: React.FC = () => {
  ...
  const categoryInfo: Record<string, { description: string; icon: string; priority: 'high' | 'medium' | 'low' }> = {
    ...
    'adj-na': { description: 'Tính từ -na', icon: 'star-outline', priority: 'medium' },
    ...
    expression: { description: 'Diễn đạt', icon: 'chatbubbles', priority: 'medium' },
      ...
      Alert.alert(
        ...
        [
          { text: 'Hủy', style: 'cancel' },
          {
            text: 'Học ngay',
            onPress: () => {
              console.log('Start studying:', categoryItem.category, vocabularyItems.length, 'words');
  ...
  const filteredCategories = categories.filter(cat => 
    filter === 'all' || cat.priority === filter
  );
  ...
  const renderFilterButton = (priority: 'all' | 'high' | 'medium' | 'low', label: string, color: string) => (
    <TouchableOpacity
      style={[
        styles.filterButton,
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          {filteredCategories.length} chủ đề • {filteredCategories.reduce((sum, cat) => sum + cat.count, 0)} từ
        </Text>
      </View>
